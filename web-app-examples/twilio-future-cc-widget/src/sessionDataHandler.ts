...
export const sessionDataHandler = {
    ...
    async getUpdatedToken(): Promise<Token> {
        ...
        try {
            newTokenData = await contactBackend<Token>("/features/web-chat/common/refresh-token", {
    ...
    fetchAndStoreNewSession: async ({ formData }: { formData: Record<string, unknown> }) => {
        ...
        try {
            newTokenData = await contactBackend<Token>("/features/web-chat/common/init-webchat", { formData });
        } catch (e) {
