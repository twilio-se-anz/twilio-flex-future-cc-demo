...
const execTwilioCli = (command) => {
  const outputRaw = shell.exec(`${command} --no-limit -o json`, {
    silent: true,
  });
  ...
  if (outputRaw.code !== 0) {
    ...
    throw new Error(
      `Failed to execute Twilio CLI command. Did you provide valid credentials? Error: ${outputRaw.stderr}`
    );
  }
...
const isMatch = (searchValue, valueToCheck, allowFuzz) => {
  ...
  if (
    ...
    return new RegExp(searchValue.slice(1, searchValue.length - 1)).test(
      valueToCheck
    );
  }
...
export const fetchServerlessDomains = () => {
  ...
  const serverlessServices = execTwilioCli(
  ...
  for (const service of serverlessServices) {
    for (const wanted in wantedDomains) {
      if (isMatch(wantedDomains[wanted].name, service.uniqueName, false)) {
        const serviceEnvironments = execTwilioCli(
          `twilio api:serverless:v1:services:environments:list --service-sid=${service.sid}`
        );
...
export const fetchTrWorkflows = (workspaceSid) => {
  ...
  const workflows = execTwilioCli(
    `twilio api:taskrouter:v1:workspaces:workflows:list --workspace-sid=${workspaceSid}`
  );
  ...
  for (const workflow of workflows) {
    for (const wanted in wantedWorkflows) {
      if (
        isMatch(wantedWorkflows[wanted].name, workflow.friendlyName, true) ||
        isMatch(wantedWorkflows[wanted].fallback, workflow.friendlyName, true)
      ) {
...
export const fetchTrWorkspaces = () => {
  ...
  for (const workspace of workspaces) {
    for (const wanted in wantedWorkspaces) {
      if (
        isMatch(wantedWorkspaces[wanted].name, workspace.friendlyName, false)
      ) {
...
export const fetchConversationsAddresses = () => {
  const type = "conversations-address";
  // If we already fetched these, no need to do it again
  if (fetchedTypes.includes(type)) {
    return;
  }
  ...
  console.log("Fetching Conversations Addresses...");

  const wantedConversationsAddresses = filterWantedVars(type);
  const conversationsAddresses = execTwilioCli(
    `twilio api:conversations:v1:configuration:addresses:list`
  );
  if (!conversationsAddresses || conversationsAddresses.length < 1) {
    return;
  }
  ...
  for (const address of conversationsAddresses) {
    for (const wanted in wantedConversationsAddresses) {
      if (
        ...
        isMatch(
          wantedConversationsAddresses[wanted].fallback,
          address.friendlyName,
          true
        )
      ) {
        resultCache[wanted] = address.sid;
      }
    }
  }
};
