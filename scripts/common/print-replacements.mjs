...
const formatName = (name) => {
  ...
  acronyms.forEach((acronym) => {
    ...
    if (name.endsWith(`_${acronym}`)) {
      formatted =
        formatted.slice(0, formatted.length - acronym.length) +
        acronym.toUpperCase();
    }
...
export default (allReplacements) => {
  ...
  for (const key in allReplacements) {
    if (
      !varNameMapping[key] ||
      varNameMapping[key].type != "serverless-domain"
    ) {
      continue;
    }
  ...
  for (const key in allReplacements) {
    if (
      !varNameMapping[key] ||
      varNameMapping[key].type != "conversations-address"
    ) {
      continue;
    }
