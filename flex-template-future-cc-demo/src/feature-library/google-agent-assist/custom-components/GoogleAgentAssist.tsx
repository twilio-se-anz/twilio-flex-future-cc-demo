...
import { TranscriptTurn } from '../types/GoogleAgentAssistTypes';
import { Task } from 'types/task-router';
import { UserIcon } from '@twilio-paste/icons/esm/UserIcon';
import { AgentIcon } from '@twilio-paste/icons/esm/AgentIcon';

export type GoogleAgentAssistTabProps = {
  props: {
    task?: Task;
  };
};
...
const GoogleAgentAssistTab: React.FC<GoogleAgentAssistTabProps> = ({ props }) => {
  ...
  useEffect(() => {
    if (props.task && props.task.taskChannelUniqueName === 'voice') {
      ...
      console.log('GAA: Using this CallSid for stream connection:', callSid);

      try {
        client.stream('FLEX_ASSIST_' + callSid).then((stream: any) => {
          setLoading(false);
          console.log('GAA: Access to stream:', stream);
          stream.on('messagePublished', (event: SyncStreamEvent) => {
            ...
            if (event?.message?.data.isFinal == true) {
              console.log('GAA: Adding to transcript', words);
        ...
        console.error('GAA: Unable to subscribe to Sync stream', error);
      }
    } else {
      console.log('GAA: Not currently on a voice task.');
    }
  }, []);
      ...
      <Stack orientation={'vertical'} spacing={'space40'}>
        ...
        <ChatLog>
          {transcript && (
            <ChatBookend>
              <ChatBookendItem>
                <strong>Twilio + Google</strong> Agent Assist
              </ChatBookendItem>
            </ChatBookend>
          )}
...
export default GoogleAgentAssistTab;
