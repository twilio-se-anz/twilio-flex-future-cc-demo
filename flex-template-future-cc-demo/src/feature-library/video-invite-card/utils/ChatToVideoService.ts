...
class ChatToVideoService extends ApiService {
  generateUrl = (identity: string, roomName: string): string =>
    `${this.serverlessProtocol}://${this.serverlessDomain}/features/video-invite-card/index.html?identity=${identity}&roomName=${roomName}`;
  ...
  generateVideoCode = async (taskSid: string): Promise<GenerateCodeReponse> => {
    return new Promise((resolve, reject) => {
      ...
      this.fetchJsonWithReject<GenerateCodeReponse>(
        `${this.serverlessProtocol}://${this.serverlessDomain}/features/video-invite-card/flex/generate-unique-code`,
        {
          method: 'POST',
  ...
  completeRoom = async (roomSid: string): Promise<CompleteRoomReponse> => {
    return new Promise((resolve, reject) => {
      ...
      this.fetchJsonWithReject<CompleteRoomReponse>(
        `${this.serverlessProtocol}://${this.serverlessDomain}/features/video-invite-card/flex/complete-room`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: this.buildBody(encodedParams),
